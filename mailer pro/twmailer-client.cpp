...
std::string inputbuffer;
   do //dieser loop liest in den buffer ein und tut das auf verscheidene arten je nachdem ob eine message geschrieben wird oder nicht
   { 
        input = ""; //falls man mehrere msg senden will, muss der input resettet werden
      printf(">> ");
      MsgDone = 0;
        if (inputMessage) { //Hier wird, falls man eine msg einlesen will, solang eingelesen, bis man eine Zeile mit . macht
                do{
        ...
        strncpy(buffer, input.c_str(), sizeof(buffer)); //conversion von string zu char array, cant be assed alles auf strings umzuschreiben
        }
        else{ //hier geschieht der normale input
        ...
        if (strcmp(buffer, "Please enter your message:\r\n") == 0) { //das ist die nachricht, die der client bekommt, bevor er meherere Lines inputten kann
                inputMessage = 1;
            }
        else{
                inputMessage = 0;
