...
void saveMessage(const std::string &sender, const std::string &recipient,
  ...
  // erhoehe um 1 und schreib wieder rein, (std::ios::trunc verursacht hierbei,
  // dass der alte index hierbei geloescht wird)
  index++;
  ...
  std::string filePath = folderName + "/" + indexStr + "_" + sender + "_" +
                         recipient + ".txt"; // filename inkludiert

  std::string mail =
      subject + "\n" + message; // Die text files haben in der ersten zeile das
                                // subject und darunter den textkoerper
...
std::string GetList(
    ...
    [[maybe_unused]] const std::string
        &listUser) { //[[maybe_unused]] wurde zwischenzeitlich gebraucht, damit
                     //der compiler nicht heult und jez isses halt da ¯\_(ツ)_/¯
  std::string StringfolderPath = "textlogs";
  const char *folderPath =
      StringfolderPath
          .c_str(); // opendir scheint nur mit const char arrays zu gehen
  int msgCount = 0;
  std::string list = "";
  ...
  while ((entry = readdir(dir))) {
    if (entry->d_type ==
        DT_REG) { // checks if it's a regular file and not a folder
      std::string fileName = entry->d_name;
      size_t pos = fileName.find_first_of('_');
      std::string part1 = fileName.substr(0, pos); // das ist der index
      fileName =
          fileName.substr(pos + 1); // nummer und _ wird vorne abgeschnitten
      pos = fileName.find_first_of('_');
      std::string sender =
          fileName.substr(0, pos); // das ist der name des absenders
      fileName =
          fileName.substr(pos + 1); // nummer und _ wird vorne abgeschnitten
      pos = fileName.find_first_of('.');
      std::string recipient =
          fileName.substr(0, pos); // das ist der name des empfängers
      std::cout << sender << std::endl;
      std::cout << recipient << std::endl;
      if (sender == username &&
          recipient ==
              listUser) { // Check if the filename matches the expected pattern
        std::string filePath =
            folderPath; // schreib den pfad des files in filePath
        filePath += "/";
        ...
        if (file) {
          ...
          if (fgets(buffer, sizeof(buffer),
                    file)) { // auslesen der ersten zeile, das passiert nur wenn
                             // da auch was steht
  ...
  std::string output =
      "There are " + std::to_string(msgCount) + " messages available:\n" + list;
...
std::string ReadAndReply([[maybe_unused]] const std::string &input_string) {
  ...
  DIR *dir = opendir(
      folder_path.c_str()); // hier finden wir erstmal den genauen filename raus
  if (dir != nullptr) {     // also falls erfolgreich geöffnet
    ...
    while ((entry = readdir(dir)) != nullptr) { // solange einträge im directory
      ...
      if (file_name.find(input_string + "_") ==
          0) { // find gibt die stelle des ersten Vorkommen des Argumentes an,
               // also is die id+ "_" am anfang falls das 0 ist
  ...
  size_t bytesRead = fread(buffer, 1, sizeof(buffer),
                           file); // in bytes read den inhalt schreiben
  fclose(file);

  if (bytesRead == 0) { // es muss ein fehler aufgetreten, falls bytesRead hier
                        // immer noch leer ist
    return "Error reading file";
  }

  return std::string(buffer,
                     bytesRead); // sonst auf string casten und raus damit
}
...
std::string DeleteMessage(const std::string &input_string) {
  ...
  DIR *dir =
      opendir(folder_path.c_str()); // das rennt recht analog zu ReadAndReply
  if (dir != nullptr) {
    struct dirent *entry;
...
void *clientCommunication(void *data) {
  ...
  int bytesReceived =
      recv(*current_socket, buffer, strlen(buffer),
           0); // schreibt den username der eingeben wurde in buffer
  ...
  do {
    size = recv(*current_socket, buffer, BUF - 1,
                0);   // hier bekommt der server die msg
    ...
    if (strcmp(buffer, "SEND") == 0) {
      ...
      buffer[size] = '\0';
      saveMessage(username, recipient, subject,
                  buffer); // recipient is hier derweil noch "server"
      ...
      int bytesReceived = recv(*current_socket, buffer, strlen(buffer),
                               0); // schreibt die nummer in buffer
      buffer[bytesReceived] = '\0';
